    ...
    if (access(FM_CTL_BUNDLED, R_OK) == 0) {
        ...
        if (fp) {
            while (fgets(line, sizeof line, fp)) {
                ...
                // ★★★ 添加 FM_SWITCHING 解析 ★★★
                else if (!strncmp(line, "FM_SWITCHING=", 13)) {
                    int switching = line[13] == '1';
                    if (!switching && g_fm_switching) {
                        // 脚本说切换已完成，强制清理状态
                        g_fm_switching = 0;
                        g_fm_switch_pin[0] = 0;
                        g_fm_switch_until = 0;
                        ...
                        unlink(FM_SWITCH_RC);
                        // 清理脚本的状态文件
                        unlink("/tmp/fm-switching");
                        unlink("/tmp/fm-target");
                        unlink("/tmp/fm-status-refresh");
                    }
                    g_fm_switching = switching;
                }
            }
            pclose(fp);
        }
